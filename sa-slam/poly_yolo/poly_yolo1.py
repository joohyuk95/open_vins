...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    b= K.sum(a)
    diagonal =  K.cast(K.sqrt(b), K.dtype(feats)) / 2.0
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    true_boxes[:,:, 5:NUM_ANGLES3 + 5:3] /= np.clip(np.expand_dims(np.sqrt(np.power(boxes_wh[:, :, 0], 2) + np.power(boxes_wh[:, :, 1], 2)) / 2.0, -1), 0.0001, 9999999)
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5):
    ...
    for layer in range(num_layers):
        ...
        diagonal = K.sqrt(K.pow(input_shape[::-1][0], 2) + K.pow(input_shape[::-1][1], 2)) / 2.0
...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        for b in range(0, out_boxes.shape[0]):
            ...
            diagonal = np.sqrt(np.power(out_boxes[b, 3] - out_boxes[b, 1], 2.0) + np.power(out_boxes[b, 2] - out_boxes[b, 0], 2.0)) / 2.0
...
if __name__ == "__main__":
    ...
    def _main():
        ...
        log_dir = '~/wh4/log/'
        classes_path = '~/wh4/wh_classes.txt'
        anchors_path = '~/wh4/wh_anchors.txt'
